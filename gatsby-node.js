      ...
      if (file.mimeType === FOLDER) {
        ...
        promises.push(new Promise(async (resolve, reject) => {
          ...
          switch (file.mimeType) {
            case GOOGLE_DOC:
              buffer = shouldExportGDocs ? await middleware(googleapi.getGDoc(file.id, token, exportDocMime)) : null;
              break;
            case GOOGLE_SHEET:
              buffer = shouldExportGSheets ? await middleware(googleapi.getGDoc(file.id, token, exportSheetMime)) : null;
              break;
            default:
          ...
          if (buffer === null) {
            ...
            fs.writeFile(dest, buffer, err => {
              if (err || buffer === null) return log(err);
              resolve(getFilenameByMime(file));
              return log(`Saved file ${getFilenameByMime(file)}`);
            });
          }
